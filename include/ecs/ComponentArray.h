...
namespace Aurora {
	// Needed to store all Component Arrays by their Point in one Map instead of
	// manually creating an Array for each Type
	struct BaseComponentArray {
		virtual ~BaseComponentArray() {}

		void OnEntityDestroyed(Entity entity) { m_EntityToComponent.erase(entity); }
	...
	public:
		void AddComponent(Entity entity, T& component) {
			if (m_EntityToComponent[entity] != nullptr) {
				std::cout << "[COMPONENTARRAY] Entity already contains Component"
					<< std::endl;
				return;
			}
		...
		void RemoveComponent(Entity entity) {
			...
			std::cout << "[COMPONENTARRAY] Entity " << entity
				<< " had no Component of this Type" << std::endl;
		}
